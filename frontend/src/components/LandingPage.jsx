...
const EstimateStatSection = ({language}) => {
  const ref = useRef(null)
  const isInView = useInView(ref, {amount: "all", once: true})
  const count = useMotionValue(0)
  const rounded = useTransform(() => Math.round(count.get()))
  const controls = animate(count, 274, {duration: 4})
  controls.pause()
  ...
  return (
    <div className="text-center space-y-4 p-2">
      <h3 className="text-3xl md:text-4xl mb-4 font-bold">{language["stats"]["title"]}</h3>
      <div className="w-full md:w-2/3 mx-auto space-y-12 md:grid md:grid-cols-2 md:space-y-0">
        <div className="mb-8 md:mb-0">
          ...
          <h3 className="text-base md:text-lg font-light px-4">{language["stats"]["household"]}</h3>
        </div>
        <div>
          <div className="w-fit mx-auto">
            <IconContext.Provider value={{ className: "w-24 h-24" }}>
...
const text = landingTextConstants[language]
  console.log(text)
  const storesCard = {
    ...text["nearbyStores"],
    linkTo: "/stores",
    ...
    <div className="min-h-screen font-nunito text-dark-green">
      ...
      <div>
        ...
        <div id="info-section" className="space-y-6 md:space-y-10">
          <Section {...storesCard} />
          <EstimateStatSection language={text} />
          <Section {...applyCard} />
        </div>
      </div>
    </div>
